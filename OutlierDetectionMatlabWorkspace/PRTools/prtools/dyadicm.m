...
% dyadic operations of the classes 'mapping' and 'datafile'.
%
% SEE ALSO
...
function b = dyadicm(a,p,q,fsize)
  ...
  if iscell(a) & ismapping(a{1})
    ...
    if isfixed(a{1})
      b = mapping(mfilename,'fixed',{{a{1},a{2},p}});
    elseif isuntrained(a{1})
      b = mapping(mfilename,'untrained',{{a{1},a{2},p},[],[]});
    elseif istrained(a{1})
      if isempty(fsize), fsize = getsize(a{1},2); end
      b = mapping(mfilename,'trained',{a{1},a{2},p},getlabels(a{1}),getsize(a{1},1));
    else
      error('Wrong type')
    end
  ...
  elseif iscell(p) & isuntrained(p{1}) % train mappings stored in p
    ...
    b = mapping(mfilename,'trained',{v1,v2,p},getlabels(v1),kin,kout);
