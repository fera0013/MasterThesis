...
function w = dtc(a, crit, chisqstopval, prune, t, nfeat)
    ...
    w = mapping('dtc', {crit, chisqstopval, prune, t, nfeat});
...
%    b = setfeatlab(b, getlabels(w));
    ...
    a = seldat(dataset(a)); % get rid of all unlabelled objects
    ...
    % Store the results:
    w = mapping('dtc', 'trained', {tree}, getlablist(a), k, c);
    w = setname(w, ['DecTree' upper(crit)]);
    w = setcost(w, cost);
  end
...
% 
% 	[CRITVAL, IDX] = IGR(FEATTYPE, MU, MK, K, MNA, NA, MAP, AP, ML, L, MR, R, COST, WEIGHTS, UV, SLI)
...
function [critval, idx] = igr(feattype, varargin) 
  ...
  switch feattype
    case 0
      [MU, MK, K, MNA, NA, MAP, AP, ML, L, MR, R, cost, weights, uv, sli] = deal(varargin{:});
...
%GINI 
% 
% 	[CRITVAL, IDX] = GINI(FEATTYPE, MU, MK, K, MNA, NA, MAP, AP, ML, L, MR, R, COST, WEIGHTS, UV, SLI)
...
% 
% 	[CRITVAL, IDX] = MISCLS(FEATTYPE, MU, MK, K, MNA, NA, MAP, AP, ML, L, MR, R, COST, WEIGHTS, UV, SLI)
