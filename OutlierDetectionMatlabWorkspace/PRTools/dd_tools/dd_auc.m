...
%   ERR = A*W*DD_AUC
%
% A new format is to apply DD_AUC as a mapping, by multiplying the
% classifier output with DD_AUC.
%
...
if (nargin<1)
	err = mapping(mfilename,'fixed',bnd);
...
% Next check if we are dealing with the extended ROC structure as it is
% delivered by dd_roc:
if isa(e,'struct')
	if isfield(e,'err')
		e = e.err;
