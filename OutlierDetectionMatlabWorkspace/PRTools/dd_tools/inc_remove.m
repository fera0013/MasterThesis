%INC_REMOVE Remove an object from the incsvdd
%
%        W = INC_REMOVE(W,N)
%
% Remove object number N from structure W (see inc_startup). N should be
% the index of the object defined in the dataset W.X. An updated
% version of W is returned.
%
% See also: incsvdd, dd_kernel, inc_startup, inc_add

% we assume we have a valid SVDD solution. That means, we have:
%  W.x, W.y
%  W.kernel, W.kptype, kpar
%  W.C
%  W.alf, W.b
%  W.setS, W.Ks,
%  W.setE, W.Ke,
...
if ~any(W.setR==c)
  ...
  while ~done
    ...
    K = dd_kernel(W.x(c,:),W.x(setD,:),W.ktype,W.kpar);
    switch W.type
        case 'svdd'
    ...
    switch situation
      ...
      case 4  % an error becomes a support object
        ...
        K = dd_kernel(W.x(j,:),W.x(setD,:),W.ktype,W.kpar);
        switch W.type
            case 'svdd'
