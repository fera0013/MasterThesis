...
% DD_MESSAGE Print formatted message
%
%    DD_MESSAGE(INFO,MSG,PARAMS)
%
...
if (nargin==1) & (isa(info,'double'))
	...
	dd_message(info,'DD_MESSAGE: The current message level is set to %d.\n',info);
	return
end
if nargin<1
	if usejava('desktop') && (DD_INFO>0)
	...
	dd_message(DD_INFO,'DD_MESSAGE: The current message level is %d.\n',abs(DD_INFO));
	return
end
